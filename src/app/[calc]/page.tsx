...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!meta) {
    return { title: 'CalcHub - Free Online Calculators', description: 'Free online calculators for finance, health, math, and everyday use.' };
  }
...
export default async function CalcPage({ params }: Props) {
  const { calc } = await params;
  const meta = calcMetaMap[calc];
  const content = toolContent[calc] || {};
  return <CalcPageClient calcSlug={calc} calcMeta={meta ? { ...meta, ...content } : undefined} />;
}
